...
# n_wrong on vektori, johon tulee TRUE jos class - prob on yli 0.5 absoluuttiselta arvoltaan. 
# Jos absolute value ylittää tuon, on malli tehnyt väärän arvion, koska jos alkoholin käyttö on korkeaa tulee arvoksi 1 ja jos prob oli alle 0.5, on erotus yli 0.5 jolloin n_wrong vektoriin merkitään TRUE. Kun n_wrong vektorista lasketaan keskiarvo, saadaan selvitettyä kuinka paljon malli antaa vääriä arvioita.

# call loss_func to compute the average number of wrong predictions in the (training) data
# Tässä tulee valituksi kaikki ne tapaukset, jossa alkoholin käyttö on high use.
loss_func(class = alc$high_use, prob = 0)
# Tässä taas kaikki, joilla ei ole high use.
loss_func(class = alc$high_use, prob = 1)
# Tässä valitaan todennäköisyydeksi mallin antama todennäköisyys korkealle käytölle. Eli mitä varmempi malli on jostain vastauksesta ja se on väärässä, sitä enemmän loss function rankaisee. Esim jos malli antaa 90 % todennäköisyyden että korkea käyttö on true ja ei olekaan, tulee n_wrong arvoksi 0.9, joka nostaa huomattavasti väärän arvion todennäköisyyttä. 
loss_func(class = alc$high_use, prob = alc$probability)
# Upper has the lowest loss function, which means that the prediction is best
```
